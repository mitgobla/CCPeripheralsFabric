...
public class TileVacuumMachine extends TileGeneric implements IPeripheralTile, Tickable {
    ...
    private void pullEntities() {
        if (this.world != null ) {
            Box box = new Box(this.getPos().add(-4.0, -4.0, -4.0), this.getPos().add(4.0, 4.0, 4.0));
            ...
            for (Entity entity : entities) {
                ...
                normVector = dirVector.subtract(entity.getPos()).normalize();
                normVector = normVector.multiply(0.12);
                entity.setVelocity(entity.getVelocity().add(normVector));
            }
        }
    }
